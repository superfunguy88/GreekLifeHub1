-- Real user messaging: profiles, connections, conversations, messages
-- Run this in Supabase SQL Editor (Dashboard → SQL Editor). If you have an existing
-- "messages" table with (channel, sender_name, body), you can drop it first:
...
-- Trigger: create or update profile when a new user signs up
...
-- User connections (friend requests / "add each other")
...
-- Profiles: any authenticated user can read (to search and add friends)
